...
from pyglm.switching_models import NegativeBinomialEmptyHDPHMM
...
def fit_with_gibbs(dataset, run, seed=None):
    ...
    for itr in xrange(N_samples):
        ...
        with gzip.open(
                os.path.join(res_dir,
                             "gibbs.empty.hdphmm.itr%04d.pkl.gz" % itr),
                "w") as f:
            cPickle.dump((samples[-1], timestamps[-1]), f, protocol=-1)
    ...
    # Save the results
    ###########################################################
    results_path = os.path.join(res_dir, "gibbs.empty.hdphmm.pkl.gz")
