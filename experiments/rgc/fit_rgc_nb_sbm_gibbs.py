...
from pyglm.models import NegativeBinomialSBMPopulation
from pyglm.utils.experiment_helper import load_data, load_results

def demo(dataset="rgc_nb_sbm_300T", run=1, seed=None):
    """
    Fit a weakly sparse
    :return:
    """
    ...
    network_hypers = {"weight_hypers":
                          {"mu_0": init_model.W.mean(axis=(0,1)),
                           "Sigma_0": np.diag(init_model.W.var(axis=(0,1)))}
                     }

    # Copy the network hypers.
    test_model = NegativeBinomialSBMPopulation(
