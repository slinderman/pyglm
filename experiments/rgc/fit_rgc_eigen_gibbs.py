...
from pyglm.models import BernoulliEigenmodelPopulation, StandardBernoulliPopulation
...
def demo(seed=None):
    ...
    test_model = BernoulliEigenmodelPopulation(
    ...
    # Initialize plots
    lns, im_net, eigen_ax = initialize_plots(test_model, S)
...
def initialize_plots(test_model, S):
    ...
    fig3 = plt.figure(3)
    eigen_ax = fig3.add_subplot(111, aspect="equal")
    test_model.network.plot(test_model.weight_model.A, ax=eigen_ax)

    plt.show()
    plt.pause(0.001)

    return lns, im_net, eigen_ax

def update_plots(itr, test_model, S, lns, im_net, eigen_ax):
    N = test_model.N
    T = S.shape[0]
    ...
    # Plot the latent embedding under the eigenmodel
    plt.figure(3)
    eigen_ax.cla()
    test_model.network.plot(test_model.weight_model.A, ax=eigen_ax)
    plt.pause(0.001)
