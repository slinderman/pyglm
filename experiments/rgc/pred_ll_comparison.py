...
def plot_pred_ll_vs_time(dataset, run, algs, Z=1.0, nbins=4):
    ...
    if 'gibbs' in results:
        ...
        # DEBUG!! Only for the prelim ER results
        # t_gibbs = np.array(results["gibbs"][1])
        # plls_gibbs = compute_pred_lls(results["gibbs"][0], test)
        # t_gibbs = t_bfgs + t_gibbs
        # t_stop = max(t_stop, t_gibbs[-1])
        # ax.semilogx(t_gibbs, (plls_gibbs - homog_pll)/Z,
        #             color=col[2], label="Gibbs", lw=1.5)
