...
from pyglm.regression import SparseScalarRegression
...
true_reg = SparseScalarRegression(N, B, eta=0.01)
X = np.random.randn(T, N*B)
y = true_reg.rvs(X=X)

# Make a test regression and fit it
test_reg = SparseScalarRegression(N, B)
...
plt.imshow(mu.reshape((npts, npts)),
           cmap="Greys", vmin=-ylim, vmax=ylim,
           alpha=0.8,
           extent=lim + tuple(reversed(lim)))
plt.scatter(X[:,0], X[:,1], c=y, vmin=-ylim, vmax=ylim)
plt.xlim(lim)
plt.ylim(lim)
plt.colorbar()
