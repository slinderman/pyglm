...
import numpy as np
cimport numpy as np
from linalg cimport _chol_update, _chol_downdate
...
def block_inverse_add_rows(Pinv, Q, R, S, symm=False):
    ...
    if symm:
        ...
        St = scipy.linalg.lapack.dpotrs(
            L, np.eye(F.shape[0]), lower=True)[0]
        Rt = -solve_triangular(F, RPi)
        Pt = Pinv - PiQ.dot(solve_triangular(F, RPi))
...
def block_inverse_remove_rows(Ainv, end, symm=False):
    """
    ...
    Pt = Ainv[:end, :end]
    Qt = Ainv[:end, end:]
    Rt = Qt.T if symm else Ainv[end:, :end]
    St = Ainv[end:, end:]
